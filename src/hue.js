...
var hue = (function () {
	...
	var RefWhiteX = 0.95047,
		 ...
		 * min and max RGB components throughout that edge (0-Red, 1-Green,
		 * 2-Blue). These values correspond to sRGB and D65 white point
		 */
		rgbPrimariesH = [
	...
	function RGBtoI32_RGB(rgb) {
		return ((rgb[0] & 0xff) << 16) | ((rgb[0] & 0xff) << 8) |
				(rgb[0] & 0xff);
	}

	function sRGBtoXYZ(srgb, xyz) {
		var r = srgb[0] / 255.0,
			g = srgb[1] / 255.0,
			b = srgb[2] / 255.0;
		xyz = xyz || [0, 0, 0];
	...
	function XYZtoSRGB(xyz, srgb) {
		var x = +xyz[0],
			y = +xyz[1],
			z = +xyz[2];
		srgb = srgb || [0, 0, 0];

		srgb[0] = ((x * mXYZtoRGB[0] + y * mXYZtoRGB[3] + z * mXYZtoRGB[6]) *
				   255 + 0.5) | 0;
		srgb[1] = ((x * mXYZtoRGB[1] + y * mXYZtoRGB[4] + z * mXYZtoRGB[7]) *
				   255 + 0.5) | 0;
		srgb[2] = ((x * mXYZtoRGB[2] + y * mXYZtoRGB[5] + z * mXYZtoRGB[8]) *
				   255 + 0.5) | 0;

		return srgb;
	}

	function XYZtoLUV(xyz, luv) {
		var x = +xyz[0],
			y = +xyz[1],
			z = +xyz[2],
			exp = Math.exp, log = Math.log,
	...
	return {
		...
		LCHtoLUV: LCHtoLUV,
		// RGB <-> LUV
		RGBtoLUV: function(rgb, luv) {
			luv = luv || [0, 0, 0];
			return XYZtoLUV(RGBtoXYZ(rgb, luv), luv);
		},
		LUVtoRGB: function(luv, rgb){
			rgb = rgb || [0, 0, 0];
			return XYZtoRGB(LUVtoXYZ(luv, rgb), rgb);
		},
		// RGB <-> LCH
		RGBtoLCH: function(rgb, lch){
			lch = lch || [0, 0, 0];
			return LUVtoLCH(XYZtoLUV(RGBtoXYZ(rgb, lch), lch), lch);
		},
		LCHtoRGB: function(lch, rgb){
			rgb = rgb || [0, 0, 0];
			return XYZtoRGB(LUVtoXYZ(LCHtoLUV(lch, rgb), rgb), rgb);
		},

		/*
		 * Find most saturated colour for a given hue
		 */
		hueMSC: hueMSC,
		/*
		 * CIEDE2000, Colour difference function
		 */
		CIEDE2000: CIEDE2000
	};
}());
